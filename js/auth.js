...
// VERSION: italky-v1.5 (SELF-CONTAINED, NO config.js import, LOOP SAFE)

// ✅ Sabitler (config.js bağımlılığı yok)
const GOOGLE_CLIENT_ID =
  "1030744341756-225hv01qs2ltugoe70c67r4m4fd83f0d.apps.googleusercontent.com";

const STORAGE_KEY = "italky_user_v1";
const BASE_DOMAIN = "https://italky-api.onrender.com";
...
function isWebView(){
  try{ if(window.Android) return true; }catch{}
  const ua = String(navigator.userAgent || "");
  return /\bwv\b|; wv\)|Android/i.test(ua);
}
...
/**
 * ✅ LOOP SAFE:
 * - Bu fonksiyon sadece index sayfasında yönlendirsin.
 * - Yanlışlıkla home'da çağrılırsa redirect loop yapmasın.
 */
export function redirectIfLoggedIn(){
  const p = String(location.pathname || "");
  const isIndexLike = (p === "/" || p === "/index.html" || p.endsWith("/index.html"));
...
async function handleGoogleResponse(res){
  try{
    ...
    // backend token opsiyonel
    try{ await fetchBackendToken(idToken); }
    catch(e){ console.warn("backend token alınamadı:", e); clearApiToken(); }
    ...
    // modal yoksa da home'a geç (kill switch)
    markTermsPending();
    location.replace(HOME_PATH);
...
export function initAuth(){
  ...
  if(!window.google?.accounts?.id) {
    console.warn("GSI not ready yet");
    return;
  }
  if(!GOOGLE_CLIENT_ID){
    console.error("GOOGLE_CLIENT_ID missing (self-contained auth.js)");
    return;
  }
